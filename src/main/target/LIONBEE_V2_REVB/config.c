...
void targetConfiguration(void) {
  ...
  featureConfigMutable()->enabledFeatures |=
      (FEATURE_TELEMETRY | FEATURE_LED_STRIP | FEATURE_OSD | FEATURE_GPS |
       FEATURE_RANGEFINDER | FEATURE_OPTICALFLOW);

  /* Configuration -> Dshot Beacon Configuration */
  beeperConfigMutable()->dshotBeaconOffFlags = 0;

  /* Configuration -> LED Strip */
  ledStripStatusModeConfigMutable()->ledConfigs[0] =
      DEFINE_LED(7, 7, 8, 0, LF(COLOR), LO(LARSON_SCANNER) | LO(THROTTLE));
  ledStripStatusModeConfigMutable()->ledConfigs[1] =
      DEFINE_LED(8, 7, 13, 0, LF(COLOR), LO(LARSON_SCANNER) | LO(THROTTLE));
  ledStripStatusModeConfigMutable()->ledConfigs[2] =
      DEFINE_LED(9, 7, 11, 0, LF(COLOR), LO(LARSON_SCANNER) | LO(THROTTLE));
  ledStripStatusModeConfigMutable()->ledConfigs[3] =
      DEFINE_LED(10, 7, 12, 0, LF(COLOR), LO(LARSON_SCANNER) | LO(THROTTLE));

  /* Modes */
  modeActivationConditionsMutable(0)->modeId = BOXARM;
  modeActivationConditionsMutable(0)->auxChannelIndex =
      AUX1 - NON_AUX_CHANNEL_COUNT;
  modeActivationConditionsMutable(0)->range.startStep =
      CHANNEL_VALUE_TO_STEP(1700);
  modeActivationConditionsMutable(0)->range.endStep =
  ...
  modeActivationConditionsMutable(1)->auxChannelIndex =
      AUX2 - NON_AUX_CHANNEL_COUNT;
  modeActivationConditionsMutable(1)->range.startStep =
      CHANNEL_VALUE_TO_STEP(900);
  modeActivationConditionsMutable(1)->range.endStep =
      CHANNEL_VALUE_TO_STEP(1300);

  modeActivationConditionsMutable(2)->modeId = BOXHORIZON;
  modeActivationConditionsMutable(2)->auxChannelIndex =
      AUX2 - NON_AUX_CHANNEL_COUNT;
  modeActivationConditionsMutable(2)->range.startStep =
      CHANNEL_VALUE_TO_STEP(1300);
  modeActivationConditionsMutable(2)->range.endStep =
      CHANNEL_VALUE_TO_STEP(1700);

  modeActivationConditionsMutable(3)->modeId = BOXFLIPOVERAFTERCRASH;
  modeActivationConditionsMutable(3)->auxChannelIndex =
      AUX3 - NON_AUX_CHANNEL_COUNT;
  modeActivationConditionsMutable(3)->range.startStep =
      CHANNEL_VALUE_TO_STEP(1700);
  modeActivationConditionsMutable(3)->range.endStep =
      CHANNEL_VALUE_TO_STEP(2100);

  modeActivationConditionsMutable(4)->modeId = BOXBLACKBOX;
  modeActivationConditionsMutable(4)->auxChannelIndex =
      AUX4 - NON_AUX_CHANNEL_COUNT;
  modeActivationConditionsMutable(4)->range.startStep =
      CHANNEL_VALUE_TO_STEP(1700);
  modeActivationConditionsMutable(4)->range.endStep =
      CHANNEL_VALUE_TO_STEP(2100);
...
#define _USER_VTX_TABLE_MAX_POWER_LEVELS 3
  ...
  uint16_t vtxTableFrequency
      [_USER_VTX_TABLE_MAX_BANDS][_USER_VTX_TABLE_MAX_CHANNELS] = {
  ...
  const char *vtxTableBandNames[_USER_VTX_TABLE_MAX_BANDS + 1] = {
      "BOSCAM A", "BOSCAM B", "BOSCAM E", "FATSHARK", "RACEBAND",
  };
...
#undef _USER_VTX_TABLE_MAX_POWER_LEVELS
  ...
  /* Motors */
  motorConfigMutable()->digitalIdleOffsetValue = 1600;
  ...
  osdElementConfigMutable()->item_pos[OSD_MAIN_BATT_VOLTAGE] =
      OSD_PROFILE_1_FLAG | OSD_POS(24, 10);
  osdElementConfigMutable()->item_pos[OSD_RSSI_VALUE] =
      OSD_PROFILE_1_FLAG | OSD_POS(1, 11);
  osdElementConfigMutable()->item_pos[OSD_ITEM_TIMER_2] =
      OSD_PROFILE_1_FLAG | OSD_POS(1, 10);
  osdElementConfigMutable()->item_pos[OSD_FLYMODE] =
      OSD_PROFILE_1_FLAG | OSD_POS(18, 10);
  osdElementConfigMutable()->item_pos[OSD_VTX_CHANNEL] =
      OSD_PROFILE_1_FLAG | OSD_POS(8, 10);
  osdElementConfigMutable()->item_pos[OSD_CURRENT_DRAW] =
      OSD_PROFILE_1_FLAG | OSD_POS(23, 11);
  osdElementConfigMutable()->item_pos[OSD_CRAFT_NAME] =
      OSD_PROFILE_1_FLAG | OSD_POS(8, 11);
  osdElementConfigMutable()->item_pos[OSD_WARNINGS] =
      OSD_PROFILE_1_FLAG | OSD_PROFILE_FLAG(2) | OSD_PROFILE_FLAG(3) |
      OSD_POS(9, 6);
  ...
  pidProfilesMutable(0)->d_min[FD_ROLL] = 39;
  ...
  pidProfilesMutable(0)->simplified_dmin_ratio = 100;
  ...
  controlRateProfilesMutable(0)->throttle_limit_type =
