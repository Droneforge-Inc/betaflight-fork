...
#define MTF_HEADER_LENGTH                                                      \
  5 // Excluding sync byte (0xEF): dev_id, sys_id, msg_id, seq, len
...
#define MTF_SET_MICOLINK_PROTOCOL_LENGTH 25
const uint8_t MTF_SET_MICOLINK_PROTOCOL_SEQUENCE[] = {
    0xef, 0x01, 0x00, 0x02, 0xdf, 0x12, 0x02, 0x00, 0x00,
    0x00, 0x00, 0x64, 0x00, 0x00, 0x04, 0x09, 0x00, 0x00,
    0x01, 0x58, 0x00, 0x00, 0x00, 0x00, 0xaf};
    ...
    case MTF_FRAME_STATE_WAIT_CKSUM: {
      ...
      if (c == cksum) {
        ...
        if (msg_id == MTF_MSG_ID_RANGE_SENSOR) {
          switch (mtfDevtype) {
          case MTF_DEVTYPE_02: {
            // Payload structure: 4 bytes time, 4 bytes dist, 1 strength, 1
            // precision, 1 status, 1 reserved, 2 velX, 2 velY, 1 quality, 1
            // flow_status, 2 reserved Indices: time=[0-3], dist=[4-7],
            // strength=[8], precision=[9], dist_status=[10], reserved=[11],
            // velX=[12-13], velY=[14-15], quality=[16], flow_status=[17]

            uint8_t distStatus = mtfPayload[10];
            uint32_t distValue = mtfPayload[4] | (mtfPayload[5] << 8) |
                                 (mtfPayload[6] << 16) | (mtfPayload[7] << 24);

            if (distStatus != 0 && distValue >= MTF_02_RANGE_MIN &&
                distValue < MTF_02_RANGE_MAX) {
...
static bool mtfDetect(optrangeDev_t *dev, uint8_t devType) {
  ...
  if (!mtfDetected) {
    const serialPortConfig_t *portConfig =
        findSerialPortConfig(FUNCTION_OPTRANGE);

    if (!portConfig) {
      return false;
    }

    mtfSerialPort = openSerialPort(portConfig->identifier, FUNCTION_OPTRANGE,
                                   NULL, NULL, 115200, MODE_RXTX, 0);
...
bool mtf02Detect(optrangeDev_t *dev) { return mtfDetect(dev, MTF_DEVTYPE_02); }
