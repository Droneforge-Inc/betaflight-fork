...
uint32_t getCrsfDesiredSpeed(void) {
  return checkCrsfCustomizedSpeed() ? baudRates[crsfSpeed.index]
                                    : CRSF_BAUDRATE;
}
...
void initCrsfMspBuffer(void) { mspRxBuffer.len = 0; }

bool bufferCrsfMspFrame(uint8_t *frameStart, int frameLength) {
  if (mspRxBuffer.len + CRSF_MSP_LENGTH_OFFSET + frameLength >
...
bool handleCrsfMspFrameBuffer(mspResponseFnPtr responseFn) {
  ...
  while (true) {
    ...
    if (handleMspFrame(&mspRxBuffer.bytes[CRSF_MSP_LENGTH_OFFSET + pos],
                       mspFrameLength, NULL)) {
      if (crsfRxIsTelemetryBufEmpty()) {
        replyPending = sendMspReply(CRSF_FRAME_TX_MSP_FRAME_SIZE, responseFn);
      } else {
...
static void crsfFinalize(sbuf_t *dst) {
  crc8_dvb_s2_sbuf_append(
      dst, &crsfFrame[2]); // start at byte 2, since CRC does not include device
                           // address and frame length
...
void crsfFrameGps(sbuf_t *dst) {
  ...
  sbufWriteU32BigEndian(
      dst, gpsSol.llh.lat); // CRSF and betaflight use same units for degrees
  sbufWriteU32BigEndian(dst, gpsSol.llh.lon);
  sbufWriteU16BigEndian(dst, (gpsSol.groundSpeed * 36 + 50) /
                                 100); // gpsSol.groundSpeed is in cm/s
  sbufWriteU16BigEndian(dst, gpsSol.groundCourse *
                                 10); // gpsSol.groundCourse is degrees * 10
  const uint16_t altitude =
      (constrain(getEstimatedAltitudeCm(), 0 * 100, 5000 * 100) / 100) +
      1000; // constrain altitude from 0 to 5,000m
...
static void crsfFrameVarioSensor(sbuf_t *dst) {
  // use sbufWrite since CRC does not include frame length
  sbufWriteU8(dst, CRSF_FRAME_VARIO_SENSOR_PAYLOAD_SIZE +
...
void crsfFrameBatterySensor(sbuf_t *dst) {
  // use sbufWrite since CRC does not include frame length
  sbufWriteU8(dst, CRSF_FRAME_BATTERY_SENSOR_PAYLOAD_SIZE +
                       CRSF_FRAME_LENGTH_TYPE_CRC);
  sbufWriteU8(dst, CRSF_FRAMETYPE_BATTERY_SENSOR);
  if (telemetryConfig()->report_cell_voltage) {
    sbufWriteU16BigEndian(
        dst, getBatteryAverageCellVoltage()); // vbat is in units of 0.01V
  } else {
  ...
  const uint8_t batteryRemainingPercentage =
...
// Due to strange OpenTX behavior of count any 0xFFFF value as incorrect, the
// maximum sending value is limited to 0xFFFE (32766 meters) in order to have
// both precision and range in 16-bit value of altitude is packed with different
// precision depending on highest-bit value. on receiving side: if MSB==0,
// altitude is sent in decimeters as uint16 with -1000m base. So, range is
// -1000..2276m. if MSB==1, altitude is sent in meters with 0 base. So, range is
// 0..32766m (MSB must be zeroed). altitude lower  than -1000m is sent as zero
// (should be displayed as "<-1000m" or something). altitude higher than 32767m
// is sent as 0xfffe (should be displayed as ">32766m" or something). range from
// 0 to 2276m might be sent with dm- or m-precision. But this function always
// use dm-precision.
static inline uint16_t calcAltitudePacked(int32_t altitude_dm) {
  ...
  if (valDm < 0)
    return 0; // too low, return minimum
  if (valDm < 0x8000)
    return valDm; // 15 bits to return dm value with offset

  return MIN((altitude_dm + 5) / 10, 0x7fffe) |
         0x8000; // positive 15bit value in meters, with OpenTX limit
}

static inline int8_t calcVerticalSpeedPacked(
    int16_t verticalSpeed) // Vertical speed in m/s (meters per second)
{
  // linearity coefficient.
  // Bigger values lead to more linear output i.e., less precise smaller values
  // and more precise big values. Decreasing the coefficient increases
  // nonlinearity, i.e., more precise small values and less precise big values.
  static const float Kl = 100.0f;

  // Range coefficient is calculated as result_max / log(verticalSpeedMax *
  // LinearityCoefficient + 1); but it must be set manually (not calculated) for
  // equality of packing and unpacking
  static const float Kr = .026f;

  int8_t sign = verticalSpeed < 0 ? -1 : 1;
  const int result32 =
      lrintf(log_approx(verticalSpeed * sign / Kl + 1) / Kr) * sign;
...
static void crsfFrameAltitude(sbuf_t *dst) {
  // use sbufWrite since CRC does not include frame length
  sbufWriteU8(dst, CRSF_FRAME_BARO_ALTITUDE_PAYLOAD_SIZE +
                       CRSF_FRAME_LENGTH_TYPE_CRC);
...
void crsfFrameHeartbeat(sbuf_t *dst) {
  sbufWriteU8(dst,
              CRSF_FRAME_HEARTBEAT_PAYLOAD_SIZE + CRSF_FRAME_LENGTH_TYPE_CRC);
...
void crsfFramePing(sbuf_t *dst) {
  sbufWriteU8(dst,
              CRSF_FRAME_DEVICE_PING_PAYLOAD_SIZE + CRSF_FRAME_LENGTH_TYPE_CRC);
...
// convert andgle in decidegree to radians/10000 with reducing angle to +/-180
// degree range
...
void crsfFrameAttitude(sbuf_t *dst) {
  sbufWriteU8(dst,
              CRSF_FRAME_ATTITUDE_PAYLOAD_SIZE + CRSF_FRAME_LENGTH_TYPE_CRC);
...
void crsfFrameRawImu(sbuf_t *dst) {
  sbufWriteU8(dst,
              CRSF_FRAME_RAW_IMU_PAYLOAD_SIZE + CRSF_FRAME_LENGTH_TYPE_CRC);
...
#if defined(USE_GPS)
  if (!ARMING_FLAG(ARMED) && featureIsEnabled(FEATURE_GPS) &&
...
void crsfFrameDeviceInfo(sbuf_t *dst) {
  ...
  sbufWriteU32BigEndian(dst, HARDWARE_VERSION_DF);
#else
  sbufWriteU32BigEndian(dst, 0x00);
#endif
#if defined(FIRMWARE_VERSION_DF)
  sbufWriteU32BigEndian(dst, FIRMWARE_VERSION_DF);
#else
  sbufWriteU32BigEndian(dst, 0x00);
#endif
...
static void crsfProcessSpeedNegotiationCmd(uint8_t *frameStart) {
  uint32_t newBaudrate = frameStart[2] << 24 | frameStart[3] << 16 |
                         frameStart[4] << 8 | frameStart[5];
  uint8_t ii = 0;
...
void speedNegotiationProcess(timeUs_t currentTimeUs) {
  if (crsfSpeed.hasPendingReply) {
  ...
  } else if (!featureIsEnabled(FEATURE_TELEMETRY) &&
             crsfRxUseNegotiatedBaud()) {
    // Send heartbeat if telemetry is disabled to allow RX to detect baud rate
    // mismatches
    ...
    if ((cmpTimeUs(currentTimeUs, lastPing) > CRSF_LINK_TYPE_CHECK_US)) {
      // Send a ping, the response to which will be a device info response
      // giving the rx serial number
      sbuf_t crsfPayloadBuf;
      sbuf_t *dst = &crsfPayloadBuf;
      crsfInitializeFrame(dst);
      crsfFramePing(dst);
    ...
    if (runLength > 1) {
      ...
      if (destRemaining >= totalBatches * CRSF_RLE_BATCH_SIZE) {
        for (unsigned int i = 1; i <= totalBatches; i++) {
          const uint8_t batchLength =
              (i < totalBatches) ? CRSF_RLE_MAX_RUN_LENGTH : remainder;
          sbufWriteU8(dest, c);
...
static void crsfFrameDisplayPortChunk(sbuf_t *dst, sbuf_t *src, uint8_t batchId,
                                      uint8_t idx) {
  uint8_t *lengthPtr = sbufPtr(dst);
  sbufWriteU8(dst, 0);
...
static void crsfFrameRangefinderTF(sbuf_t *dst) {
  // use sbufWrite since CRC does not include frame length
  sbufWriteU8(dst, CRSF_FRAME_RANGEFINDER_TF_PAYLOAD_SIZE +
                       CRSF_FRAME_LENGTH_TYPE_CRC);
  ...
  sbufWriteU16BigEndian(dst, (distance > 0) ? constrain(distance, 0, 65535)
                                            : UINT16_MAX);
...
static void crsfFrameMotorRpm(sbuf_t *dst) {
  ...
  const uint8_t payloadSize =
      hasDshot ? (1 + 4 * 3)
               : 4; // with dshot: 1 pole + 4*(1 pwm + 2 erpm), without: 4 pwm
  ...
  for (int i = 0; i < 4; i++) {
    uint8_t motorOutput = scaleRange(
        constrain(lrintf(motor[i]), DSHOT_MIN_THROTTLE, DSHOT_MAX_THROTTLE),
...
static uint8_t mspRequestOriginID =
...
static void crsfSendMspResponse(uint8_t *payload, const uint8_t payloadSize) {
  ...
  sbufWriteU8(
      dst,
      payloadSize +
          CRSF_FRAME_LENGTH_EXT_TYPE_CRC);   // size of CRSF frame (everything
                                             // except sync and size itself)
  sbufWriteU8(dst, CRSF_FRAMETYPE_MSP_RESP); // CRSF type
  sbufWriteU8(dst, mspRequestOriginID); // response destination must be the same
                                        // as request origin in order to
                                        // response reach proper destination.
  sbufWriteU8(dst,
              CRSF_ADDRESS_FLIGHT_CONTROLLER); // origin is always this device
...
void initCrsfTelemetry(void) {
  // check if there is a serial port open for CRSF telemetry (ie opened by the
  // CRSF RX) and feature is enabled, if so, set CRSF telemetry enabled
...
bool checkCrsfTelemetryState(void) { return crsfTelemetryEnabled; }
...
void handleCrsfTelemetry(timeUs_t currentTimeUs) {
  ...
  // This needs to be done at high frequency, to enable the RX to send the
  // telemetry frame in between the RX frames.
  crsfRxSendTelemetryData();
  ...
  if (crsfDisplayPortIsReady()) {
    ...
    if (crsfDisplayPortScreen()->updated) {
      ...
      uint16_t screenSize =
          crsfDisplayPortScreen()->rows * crsfDisplayPortScreen()->cols;
      uint8_t *srcStart = (uint8_t *)crsfDisplayPortScreen()->buffer;
      uint8_t *srcEnd =
          (uint8_t *)(crsfDisplayPortScreen()->buffer + screenSize);
      src = sbufInit(&displayPortSbuf, srcStart, srcEnd);
      displayPortBatchId =
          (displayPortBatchId + 1) % CRSF_DISPLAYPORT_BATCH_MAX;
      batchIndex = 0;
    }

    // Wait between successive chunks of displayport data for CMS menu display
    // to prevent ELRS buffer over-run if necessary
    if (src && sbufBytesRemaining(src) &&
        (cmpTimeUs(currentTimeUs, batchLastTimeUs) >
